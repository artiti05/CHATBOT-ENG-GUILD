...
from core.config import PDFS_DIR, TEXTS_DIR, PARSED_OUTPUT_DIR, NEW_UPLOADS_DIR
...
async def upload_file(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    return {"status": "processing_started", "filename": file.filename, "message": "File uploaded and background processing started."}

@router.delete("/files/{filename}", dependencies=[Depends(verify_admin_key)])
async def delete_file(filename: str):
    # This requires excluding it from the registry, which deletes it from ChromaDB
    try:
...
async def reingest_files(req: ReingestRequest, background_tasks: BackgroundTasks):
    ...
    # 2. Exclude them first to avoid duplicates
