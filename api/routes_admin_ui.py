"""
Admin Panel UI — browser page for managing knowledge-base documents.

Serves a single-page panel at /admin that calls the admin API in
api/routes_admin.py (mounted at /api/admin):

    GET    /api/admin/files              — list indexed documents
    POST   /api/admin/files/upload       — upload a pdf/txt/md (202, background ingest)
    DELETE /api/admin/files/{filename}   — remove a document from the KB
    POST   /api/admin/files/re-ingest    — re-process existing files (202)

Auth: the same X-API-Key header used across the app, validated against
ADMIN_API_KEY (see api/dependencies.py). The key is entered once in the
panel and kept in the browser's localStorage — it is never embedded in
this HTML or in the repository.
"""
