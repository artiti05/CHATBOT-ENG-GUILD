...
def extract_query_and_k(
    req: Optional[BaseModel], query_param: Optional[str], top_k_param: int
):
...
@router.api_route(
    "/search", methods=["GET", "POST"], dependencies=[Depends(verify_user_key)]
)
def search_kb(
...
@router.api_route(
    "/chat", methods=["GET", "POST"], dependencies=[Depends(verify_user_key)]
)
def chat_with_kb(
...
@router.api_route(
    "/query", methods=["GET", "POST"], dependencies=[Depends(verify_user_key)]
)
def query_alias(
    req: Optional[ChatRequest] = None,
    query: Optional[str] = Query(None),
...
def chat_with_kb_stream(
    ...
    return StreamingResponse(
        chatbot.answer_question_stream(
            query=q,
            history=hist_dicts,
            top_k=k,
            session_id=session_id,
