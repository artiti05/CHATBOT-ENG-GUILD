...
_FLAG_OPEN_RE = re.compile(r'<\s*flag\s*>', re.IGNORECASE)
_FLAG_CLOSE_RE = re.compile(r'</\s*flag\s*>', re.IGNORECASE)
_FLAG_PAIR_RE = re.compile(r'<\s*flag\s*>.*?</\s*flag\s*>', re.IGNORECASE | re.DOTALL)
_FLAG_TAG_RE = re.compile(r'<\s*/?\s*flag\s*>', re.IGNORECASE)


def detect_moderation_flag(text: str) -> bool:
    if not text:
        return False
    return bool(_FLAG_PAIR_RE.search(text) or _FLAG_TAG_RE.search(text) or "offensive_detected" in text.lower())
...
def strip_reasoning(text: str) -> str:
    ...
    text = _FLAG_TAG_RE.sub('', text)
    return text.strip()


class ReasoningStreamFilter:
    _MAX_TAG_LEN = len('</reasoning>') + 12
    ...
    def feed(self, token: str) -> str:
        ...
        while True:
            if self._in_think:
                ...
                m = _FLAG_CLOSE_RE.search(self._buf)
                if m:
                    self._buf = self._buf[m.end():]
                    self._in_flag = False
                ...
                m_think = _THINK_OPEN_RE.search(self._buf)
                m_flag = _FLAG_OPEN_RE.search(self._buf)

                earliest = None
                earliest_type = None
                if m_think and m_flag:
                    if m_think.start() < m_flag.start():
                        earliest = m_think
                        earliest_type = "think"
                    else:
                        earliest = m_flag
                ...
                if earliest:
                    out.append(self._buf[:earliest.start()])
                    self._buf = self._buf[earliest.end():]
    ...
    def flush(self) -> str:
        remainder = "" if (self._in_think or self._in_flag) else self._buf
        self._buf = ""
        self._in_think = False
        self._in_flag = False
        if remainder and (_THINK_OPEN_RE.match(remainder.strip()) or _FLAG_OPEN_RE.match(remainder.strip())):
            return ""
        remainder = _THINK_CLOSE_RE.sub('', remainder)
        remainder = _FLAG_CLOSE_RE.sub('', remainder)
        return remainder
...
def clean_formatting(text: str) -> str:
    ...
    cleaned = _FLAG_PAIR_RE.sub('', cleaned)
    cleaned = _FLAG_TAG_RE.sub('', cleaned)
...
class ResponseGeneratorAgent:
    def __init__(self, ollama_url: str = OLLAMA_URL, model: str = OLLAMA_CHAT_MODEL):
        ...
        self.model = model
        self.last_flag_detected: bool = False
    ...
    def build_prompt(self,
        ...
        if is_english:
            system_prompt = (
                ...
                "  * Prepend the hidden tag <flag>offensive_detected</flag> at the start of your response.\n"
                ...
                "Answer:\n"
                "<flag>offensive_detected</flag>I am an information assistant dedicated to the Jordan Engineers Association services, regulations, and procedures. How may I assist you with your guild inquiries today?\n"
                "---\n"
                "User Question: \"You thieves, why are you taking my money? How much is the annual renewal fee?\"\n"
                "Answer:\n"
                "<flag>offensive_detected</flag>The annual subscription fee for practicing engineers is 30 JOD per year.\n"
                ...
                "Answer:\n"
                "<flag>offensive_detected</flag>To track delayed engineering blueprint approvals and submit inquiries:\n"
                ...
                "Answer:\n"
                "<flag>offensive_detected</flag>To follow up on pending training or workplace center transfer requests:\n"
                ...
                "Answer:\n"
                "<flag>offensive_detected</flag>Adding family dependents (spouse and children) to the health insurance is processed during designated enrollment windows (January and July each year).\n"
                ...
                "Answer:\n"
                "<flag>offensive_detected</flag>Authentication of engineering experience certificates is conducted at the Technical and Professional Affairs Department at JEA Headquarters (Amman - Shmeisani) or governorate branches.\n"
                ...
                "Answer:\n"
                "<flag>offensive_detected</flag>The mandatory retirement age to qualify for full pension benefits from the JEA Pension Fund is:\n"
            ...
            system_prompt = (
                ...
                f"  * أضف الوسم <flag>offensive_detected</flag> في بداية إجابتك.\n"
                ...
                f"الإجابة:\n"
                f"<flag>offensive_detected</flag>أنا مساعد معلومات مخصص لخدمات وأنظمة نقابة المهندسين الأردنيين. أنا هنا لمساعدتك في أي استفسار نقابي أو إجراء رسمي بكل مهنية واحترام. كيف يمكنني خدمتك؟\n"
                f"---\n"
                f"سؤال المستخدم: \"يا نصابين ليش بتسرقوا فلوسنا؟ قديش رسوم الاشتراك السنوي للمهندس؟\"\n"
                f"الإجابة:\n"
                f"<flag>offensive_detected</flag>رسم الاشتراك السنوي للمهندس الممارس هو 30 ديناراً أردنياً سنوياً.\n"
                ...
                f"الإجابة:\n"
                f"<flag>offensive_detected</flag>لمتابعة حالة مصادقة المخططات الهندسية المتأخرة والاعتراض:\n"
                ...
                f"الإجابة:\n"
                f"<flag>offensive_detected</flag>لمتابعة طلب نقل مركز التدريب أو العمل المعلق:\n"
                ...
                f"الإجابة:\n"
                f"<flag>offensive_detected</flag>إضافة أفراد العائلة (الزوج/الزوجة والأبناء) للتأمين الصحي يتم خلال فترات الاشتراك المحددة (شهر كانون الثاني وشهر تموز من كل عام).\n"
                ...
                f"الإجابة:\n"
                f"<flag>offensive_detected</flag>تصديق شهادات الخبرة الهندسية يتم لدى دائرة الشؤون الفنية والمهنية بمقر النقابة الرئيسي (عمان - الشميساني) أو في فروع المحافظات.\n"
                ...
                f"الإجابة:\n"
                f"<flag>offensive_detected</flag>سن التقاعد الإلزامي لاستحقاق الراتب التقاعدي الكامل من صندوق تقاعد نقابة المهندسين هو:\n"
        ...
        for model_tag in models_to_try:
            try:
                ...
                if res.status_code == 200:
                    ...
                    if raw_content:
                        is_flagged = detect_moderation_flag(raw_content)
                        self.last_flag_detected = is_flagged
                        return clean_formatting(raw_content), True
            except Exception:
                continue
        ...
        for model_tag in models_to_try:
            try:
                ...
                if res.status_code == 200:
                    ...
                    if tail:
                        yield tail
                    self.last_flag_detected = reasoning_filter.flag_detected
                    return
            except Exception:
                continue
